...
class GapAttention(nn.Module):
    def __init__(self,input_dim,daytime,gapdis,var_num):
    ...
    def forward(self,x, start_p, selected_idx):
        if not selected_idx:
            _,_,N=x.size()
            x_pre=x
            x=x.permute(0,2,1)
        ...
        x_pre=x
        x=x.permute(0,2,1)
        dim_var = int(x_pre.size(2)) 
        dim_patch=int(self.daytime)

        new_length = self.input_dim // self.daytime * self.gapdis  
        
        output_x = torch.empty((int(x_pre.size(0)), dim_var, new_length))

        for m in range(self.input_dim//self.daytime):
            start_idx = m * self.daytime
            end_idx = start_idx + self.gapdis  
            output_x[:, :, m * self.gapdis: (m + 1) * self.gapdis] = x[:, :, start_idx:end_idx]
        ...
        x_gap=[]
        x_gaps=[]
        sampled_x_days = [0]*self.var_num
        for day_index, x_day in enumerate(x_days):
            
            XB, XN, XT = x_day.size()
            x_day_reshape = x_day.view(self.var_num, int(XB/self.var_num), XN, XT)

            for k in range(self.var_num): 

                variable = x_day_reshape[k, :, :, :]

                if k in selected_idx:
                    sampled_variable = variable[:, :, int((dim_patch/2-self.gapdis/2)):int((dim_patch/2+self.gapdis/2))]
                    
                else:
                    sampled_variable = variable[:, :, start_p[day_index][k]:start_p[day_index][k]+self.gapdis]

                sampled_x_days[k]=sampled_variable
            final_sampled_x_days = torch.cat(sampled_x_days, dim=0)


            x_gap.append(final_sampled_x_days)
            
        x_gap = torch.cat(x_gap, dim=1)  
        
        x_gaps.append(x_gap)

        
        for k in range(len(x_gaps)):
            x_gaps[k],_=self.multiattention_layers[k](x_gaps[k],x_gaps[k],x_gaps[k])

            for m in range(self.input_dim//self.daytime):
                output_x[:, :, m * self.gapdis: (m + 1) * self.gapdis]=x_gaps[k][:,m*N:(m+1)*N,:]

                # x[:,:,m*self.daytime+k*self.gapdis:m*self.daytime+(k+1)*self.gapdis]=x_gaps[k][:,m*N:(m+1)*N,:]

        output_x=output_x.permute(0,2,1)
...
class PatchAttention(nn.Module):
    ...
    def forward(self,x):
        ...
        assert self.input_dim % self.daytime == 0, "input_dim error"
